...
def simulate_camera_response(image_list: list[np.ndarray]) -> np.ndarray:
    """
    Simulate camera response for long exposure:
    - Convert to linear domain
    - Accumulate and average
    - Convert back to sRGB

    Args:
        image_list: List of RGB images in uint8, shape (H, W, 3)

    Returns:
        Averaged image (sRGB), shape (H, W, 3), dtype uint8
    """
    linear_sum = None
    for img in image_list:
        ...
        if linear_sum is None:
            ...
            linear_sum += linear
    avg_linear = linear_sum / len(image_list)
    return linear_to_srgb(avg_linear)
